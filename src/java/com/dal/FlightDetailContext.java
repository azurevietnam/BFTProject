...
public class FlightDetailContext extends DBContext {
    ...
    public List<FlightDetail> searchFlightDetailByFlightID(String flightID) throws Exception {
        List<FlightDetail> lst = new ArrayList<>();
        String sql = "select * from FLIGHT_DETAILS where flight_id='" + flightID + "'";
        ResultSet rs = getConnection().prepareStatement(sql).executeQuery();
        while (rs.next()) {
            FlightDetail detail = new FlightDetail();
